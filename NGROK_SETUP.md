...
> Si ves 404 en `/`, confirma que estás apuntando al puerto 8000 y no a otro servicio; el backend ahora responde `{"status": "ok"}` en `/` y `{"status": "healthy"}` en `/health`.
...
3. Comprueba que el túnel responde visitando `https://<subdominio>.ngrok-free.app/health` (debe devolver JSON `{"status": "healthy"}`) y luego abre el dashboard apuntando a esa URL. Deberías ver los eventos si la red no bloquea salidas HTTPS.
